    ...
    <>
      <Flex alignItems="center">
        ...
        <Box flex={1} ml={3}>
          <Flex alignItems="center" justifyContent="space-between">
            ...
            <Text color="gray">
              {dayjs(details?.msg_in_appchain.timestamp).format('YYYY-MM-DD HH:mm:ss')}
            </Text>
          </Flex>
          <Flex alignItems="center" justifyContent="space-between" fontSize="sm">
            <Text color="gray">Extrinsic</Text>
            <Link maxW="320px" isExternal href={`${network?.octopus.explorerUrl}/?appchain=${appchainId}#extrinsics/${details?.msg_in_appchain.extrinsic_id}`} 
      ...
      <Flex alignItems="flex-start">
        ...
        <Box flex={1} ml={3}>
          ...
          <List mt={3} spacing={3}>
            {
              details?.processing_msgs_in_near.concat(
                details?.staging_msgs_in_near
              ).filter(i => !!i).map(item => (

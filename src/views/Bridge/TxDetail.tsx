...
export const TxDetail: React.FC<TxDetailProps> = ({ onDrawerClose }) => {
  ...
  const { data: transaction } = useSWR(txId ? `bridge-helper/bridgeTx/${txId}` : null, { refreshInterval: 1000 });

  const { hasCopied: hasTxIdCopied, onCopy: onTxIdCopy } = useClipboard(transaction?.summary.id);

  const [isAppchainSide, appchainId] = useMemo(() => [
    transaction?.summary.direction === 'appchain_to_near',
    transaction?.summary.appchain_name.replace(`${global.network?.near.networkId}-`, '')
  ], [transaction]);
    ...
    <>
      ...
      <DrawerBody pb={6}>
        <Box bg={grayBg} borderRadius="lg" pl={3} pr={3}>
          <Flex alignItems="center" justifyContent="space-between" pt={2} pb={2}>
            <Text color="gray">Transaction Id</Text>
            <HStack maxW="70%">
              <Text whiteSpace="nowrap" overflow="hidden" textOverflow="ellipsis">{transaction?.summary.id}</Text>
            ...
            <HStack>
              {
                ...
                <Link as={RouterLink} to={`/appchains/${appchainId}`}>
                  <HStack spacing={1}>
                    ...
                    <Text fontSize="sm">{isAppchainSide ? appchainId : 'NEAR'}</Text>
                  </HStack>
                </Link> :
                <HStack spacing={1}>
                  <Avatar boxSize={5} src={
          ...
          <Flex alignItems="center" justifyContent="space-between" pt={2} pb={2}>
            ...
            <HStack>
              <Heading fontSize="md" whiteSpace="nowrap" overflow="hidden" textOverflow="ellipsis">
                {
                  appchain ?
                  DecimalUtil.beautify(DecimalUtil.fromString(transaction?.summary.amount.replaceAll(',', ''), appchain?.appchain_metadata?.fungible_token_metadata?.decimals)) :
                  '-'
                }
              </Heading>
          ...
          <Flex alignItems="center" justifyContent="space-between" pt={3} pb={3}>
        ...
        <SlideFade in={!!transaction}>
          <Box mt={6}>
            {
              isAppchainSide ?
              <ProcessFromAppchain data={transaction} network={global.network} /> :

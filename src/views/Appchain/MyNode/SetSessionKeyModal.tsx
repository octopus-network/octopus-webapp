...
export const SetSessionKeyModal: React.FC<SetSessionKeyModalProps> = ({ isOpen, onClose, appchainApi }) => {
  ...
  return (
    <BaseModal 
      ...
      maxW="520px">
      {
            ...
            <List>
              {
                accounts.map(account => (
                  <Box p={2} _hover={{ background: bg }} key={account.address} borderRadius="lg" cursor="pointer" onClick={() => onChooseAccount(account)}>
                    <HStack w="calc(100% - 100px)">
                      ...
                      <VStack spacing={0} alignItems="flex-start" w="100%">
                        ...
                        <Text variant="gray" fontSize="xs" w="100%" whiteSpace="nowrap" overflow="hidden" textOverflow="ellipsis">
                          {account.address}
                        </Text>
                      </VStack>
                    </HStack>
        ...
        <>
        <List spacing={4}>
          <Flex p={3} bg={bg} borderRadius="lg" cursor="pointer" justifyContent="space-between" alignItems="center" onClick={setIsInAccountsPage.on}>
            {
              !currentAccount ?
              ...
              <>
                <HStack w="calc(100% - 100px)">
                  ...
                  <VStack spacing={1} alignItems="flex-start" w="100%">
                    <Heading fontSize="lg">{currentAccount.meta?.name || 'No Name'}</Heading>
                    <Text variant="gray" fontSize="sm" w="100%" whiteSpace="nowrap" overflow="hidden" textOverflow="ellipsis">
          ...
          <FormControl mt={2}>
            <Input type="text" placeholder="Session key" autoFocus onChange={e => onKeyChange(e.target.value)} />
            <FormHelperText>
              Session Key is usually a set of hex strings, you can get it from the node you deployed
              <Link href="https://docs.oct.network/maintain/validator-set-session-keys.html" variant="blue-underline" isExternal ml={2}>How to get?</Link>
            </FormHelperText>
          </FormControl>
        </List>
        <Box mt={8}>

...
import { Flex, Heading, Grid, Button, GridItem } from "@chakra-ui/react"
...
export const UnbondedValidatorRow: React.FC<UnbondedValidatorRowProps> = ({
  ...
  const { data: delegatorRewards } = useSWR<RewardHistory[]>(
    appchainId && validatorSetHistoryEndIndex && accountId
      ? `rewards/${id}/${appchainId}/${accountId}/${validatorSetHistoryEndIndex}`
      : null
  )

  return delegatorRewards?.length ? (
    <Grid
      templateColumns={{
        base: "repeat(5, 1fr)",
        md: "repeat(8, 1fr)",
      ...
      gap={2}
    >
      <GridItem colSpan={2} w="100%" opacity={0.5}>
        <Heading
          fontSize="md"
          whiteSpace="nowrap"
      ...
      <GridItem colSpan={2} textAlign="center" opacity={0.5}>
        -
      </GridItem>
      <GridItem
        colSpan={2}
      ...
      <GridItem colSpan={1}>
        <Button
          ...
          onClick={() => onClaimRewards(id, delegatorRewards)}
        >
          Claim Rewards
        </Button>
      </GridItem>

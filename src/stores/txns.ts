...
const localStorageTxns = window.localStorage.getItem('OCTOPUS_BRIDGE_TXNS') || null;

export const useTxnsStore = create((set: any, get: any): TxnsStore => ({
  ...
  updateTxn: (appchainId: string, txn: BridgeHistory) => {
    ...
    set({ txns: newTxns });
    window.localStorage.setItem('OCTOPUS_BRIDGE_TXNS', JSON.stringify(newTxns));
  },
